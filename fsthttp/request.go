...
func (req *Request) Send(ctx context.Context, backend string) (*Response, error) {
	...
	abiReq, abiReqBody, err := req.constructABIRequest()
	if err != nil {
		return nil, err
	}
	...
	if streaming {
		go req.copyBody(abiReqBody, errc)
		abiPending, err = abiReq.SendAsyncStreaming(abiReqBody, backend)
	} else {
		req.copyBody(abiReqBody, errc)
		abiPending, err = abiReq.SendAsync(abiReqBody, backend)
	}
	if err != nil {
...
func (req *Request) copyBody(abiReqBody *fastly.HTTPBody, errc chan<- error) {
	var (
	...
	if shouldCopy {
		...
		if copyErr == nil {
			...
			errc <- maybeWrap(abiReqBody.Abandon(), "abandon request body")
		}
	} else {
		errc <- maybeWrap(abiReqBody.Close(), "close request body")
	}
...
func (req *Request) constructABIRequest() (*fastly.HTTPRequest, *fastly.HTTPBody, error) {
